...
class ImageBatcher:
    ...
    def __init__(self, input, shape, dtype, max_num_images=None, exact_batches=False, config_file=None):
        ...
        self.input_color_format = self.det2_cfg.INPUT.FORMAT
        self.pixel_mean = np.asarray(self.det2_cfg.MODEL.PIXEL_MEAN, dtype=np.float32)
        if self.input_color_format not in {"RGB", "BGR"}:
            raise ValueError(f"Unsupported INPUT.FORMAT: {self.input_color_format}")
    ...
    def preprocess_image(self, image_path):
        ...
        * Resizes and pads the image to fit the input size.
        ...
        # Padding happens before graph normalization, so use the configured model
        # mean and obtain zeros after the graph's Sub/Div preprocessing.
        pad_rgb = self.pixel_mean
        if self.input_color_format == "BGR":
            pad_rgb = pad_rgb[::-1]
        image, scale = resize_pad(image, tuple(int(round(x)) for x in pad_rgb))
