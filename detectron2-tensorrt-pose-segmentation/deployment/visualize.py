...
def visualize_detections(image_path, output_path, detections, labels=None, iou_threshold=0.5):
    ...
    for d in detections:
        ...
        if isinstance(d['mask'], np.ndarray) and d['mask'].shape == (28, 28):
            ...
            det_width = max(1, round(d['xmax'] - d['xmin']))
            det_height = max(1, round(d['ymax'] - d['ymin']))
        ...
        for keypoint in np.asarray(d.get('keypoints', [])):
            if len(keypoint) < 2:
                continue
