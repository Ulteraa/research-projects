...
class DET2GraphSurgeon:
    def __init__(self, saved_model_path, config_file, weights):
        ...
        self.num_keypoints = self.det2_cfg.MODEL.ROI_KEYPOINT_HEAD.NUM_KEYPOINTS
        self.keypoint_ROIAlign_pooled_size = self.det2_cfg.MODEL.ROI_KEYPOINT_HEAD.POOLER_RESOLUTION
        self.keypoint_ROIAlign_sampling_ratio = self.det2_cfg.MODEL.ROI_KEYPOINT_HEAD.POOLER_SAMPLING_RATIO
        self.keypoint_ROIAlign_type = self.det2_cfg.MODEL.ROI_KEYPOINT_HEAD.POOLER_TYPE
        self.keypoint_out_res = 28

        if not self.det2_cfg.MODEL.MASK_ON or not self.det2_cfg.MODEL.KEYPOINT_ON:
            raise ValueError("The conversion requires MODEL.MASK_ON and MODEL.KEYPOINT_ON")
        ...
        log.info("Number of keypoints is {}".format(self.num_keypoints))
    ...
    def get_anchors(self, sample_image):
        ...
        a fixed square dimension that every single image in dataset must follow, since currently
        ...
        if self.height != self.width:
            raise ValueError("This graph-surgery path currently requires a square input")
        imagelist_images = ImageList.from_tensors(images, self.height)
    ...
    def update_preprocessor(self, batch_size):
        ...
        sub_const = np.expand_dims(
            np.asarray(self.det2_cfg.MODEL.PIXEL_MEAN, dtype=np.float32), axis=(1, 2)
        )
    ...
    def process_graph(self, anchors, first_nms_threshold=None, second_nms_threshold=None):
        ...
        def roi_heads(rpn_outputs, p2, p3, p4, p5, second_nms_threshold):
            ...
            # Rebuild the keypoint ROIAlign path after the final NMS, then reconnect
            # it to the traced Detectron2 keypoint head.
            keypoint_pooler_output = self.ROIAlign(
                nms_outputs[1],
                p2,
                p3,
                p4,
                p5,
                self.keypoint_ROIAlign_pooled_size,
                self.keypoint_ROIAlign_sampling_ratio,
                self.keypoint_ROIAlign_type,
                self.second_NMS_max_proposals,
                "keypoint_pooler",
            )
            keypoint_pooler_shape = np.asarray(
                [
            ...
            keypoint_pooler_reshape = self.graph.op_with_const(
                "Reshape",
                "keypoint_pooler/reshape",
                keypoint_pooler_output,
                keypoint_pooler_shape,
            )
            keypoint_head_conv = self.graph.find_node_by_op_name(
                "Conv", "/roi_heads/keypoint_head/conv_fcn1/Conv"
            )
            keypoint_head_conv.inputs[0] = keypoint_pooler_reshape[0]

            keypoint_output = self.graph.find_node_by_op_name(
                "ConvTranspose", "/roi_heads/keypoint_head/score_lowres/ConvTranspose"
            )
            keypoint_output_shape = np.asarray(
                [
                    self.batch_size,
                    self.second_NMS_max_proposals,
                    self.num_keypoints,
                    self.keypoint_out_res,
                    self.keypoint_out_res,
                ],
                dtype=np.int64,
            )
            keypoint_heatmaps = self.graph.op_with_const(
                "Reshape",
                "keypoint_head/final_reshape",
                keypoint_output.outputs[0],
                keypoint_output_shape,
            )[0]
            keypoint_heatmaps.dtype = np.float32
            keypoint_heatmaps.name = "detection_keypoint_heatmaps"
            ...
            return nms_outputs, final_graph_reshape_node[0], keypoint_heatmaps

        # The graph lookup names assume Detectron2's standard FPN/RPN/ROI-head export.
        p2, p3, p4, p5 = backbone()
        rpn_outputs = proposal_generator(anchors, first_nms_threshold)
        box_head_outputs, mask_head_output, keypoint_heatmaps = roi_heads(
            rpn_outputs, p2, p3, p4, p5, second_nms_threshold
        )
        # Detection, instance-mask, and per-keypoint heatmap outputs.
        self.graph.outputs = box_head_outputs + [mask_head_output, keypoint_heatmaps]
        self.sanitize()
