...
from keypoints import decode_keypoint_heatmaps
...
class TensorRTInfer:
    ...
    def infer(self, batch, scales=None, nms_threshold=None):
        ...
        if len(outputs) != 6:
            raise RuntimeError(
                f"Expected six outputs (boxes, masks, and keypoints), got {len(outputs)}"
            )
        keypoint_heatmaps = outputs[5]

        detections = []
        for i in range(self.batch_size):
            detections.append([])
            for n in range(int(np.asarray(nums[i]).reshape(-1)[0])):
                # Select a mask.
                mask = masks[i][n]

                # Calculate scaling values for bboxes.
                scale_y = self.inputs[0]['shape'][2] / scales[i]
                scale_x = self.inputs[0]['shape'][3] / scales[i]

                if nms_threshold and scores[i][n] < nms_threshold:
                    continue

                # EfficientNMS returns corner coordinates as x1, y1, x2, y2.
                xmin = boxes[i][n][0] * scale_x
                ymin = boxes[i][n][1] * scale_y
                xmax = boxes[i][n][2] * scale_x
                ymax = boxes[i][n][3] * scale_y
                keypoints = decode_keypoint_heatmaps(
                    keypoint_heatmaps[i][n], [xmin, ymin, xmax, ymax]
                )
    ...
    for batch, images, scales in batcher.get_batch():
        ...
        for i in range(len(images)):
            ...
            for d in detections[i]:
                ...
                line.extend(np.asarray(d['keypoints']).reshape(-1).tolist())
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--labels",
        default=None,
        help="Optional comma-separated class labels; class IDs are shown by default",
    )
