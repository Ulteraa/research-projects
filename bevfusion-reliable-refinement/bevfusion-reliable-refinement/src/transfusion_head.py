...
# Modified for the reliability-aware fusion and proposal-level refinement research prototype.
import copy
...
@MODELS.register_module()
class ReliabilityAwareFuser(nn.Module):
    def __init__(self, in_channels: List[int], out_channels: int, hidden_channels: int = 128):
        super().__init__()
        assert len(in_channels) == 2, f'Expected [img, lidar], got {len(in_channels)}'

        img_c, lidar_c = in_channels

        self.img_proj = nn.Sequential(
            nn.Conv2d(img_c, hidden_channels, 1, bias=False),
            nn.BatchNorm2d(hidden_channels),
            nn.ReLU(True),
        )
        self.lidar_proj = nn.Sequential(
            nn.Conv2d(lidar_c, hidden_channels, 1, bias=False),
            nn.BatchNorm2d(hidden_channels),
            nn.ReLU(True),
        )

        self.cam_rel = nn.Sequential(
        ...
        self.gate = nn.Sequential(
            nn.Conv2d(hidden_channels * 2 + 2, hidden_channels, 3, padding=1, bias=False),
            nn.BatchNorm2d(hidden_channels),
            nn.ReLU(True),
            nn.Conv2d(hidden_channels, 1, 1)
        )
        ...
        self.out = nn.Sequential(
            nn.Conv2d(hidden_channels * 2, out_channels, 3, padding=1, bias=False),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(True),
        )

        self.residual = nn.Conv2d(hidden_channels, out_channels, 1, bias=False)

    def forward(self, inputs: List[torch.Tensor]) -> torch.Tensor:
        img_feat, lidar_feat = inputs

        img_feat = self.img_proj(img_feat)
        lidar_feat = self.lidar_proj(lidar_feat)

        # Simple LiDAR occupancy proxy from feature magnitude.
        lidar_occ = torch.linalg.vector_norm(lidar_feat, ord=2, dim=1, keepdim=True)
        lidar_occ = lidar_occ / (lidar_occ.amax(dim=(-2, -1), keepdim=True).clamp(min=1e-6))

        # Learned camera reliability map.
        cam_rel = torch.sigmoid(self.cam_rel(img_feat))

        gate = torch.sigmoid(self.gate(torch.cat([img_feat, lidar_feat, lidar_occ, cam_rel], dim=1)))

        fused = self.out(torch.cat([gate * img_feat, (1.0 - gate) * lidar_feat], dim=1))
        fused = fused + self.residual(lidar_feat)
        return fused
@MODELS.register_module()
...
@MODELS.register_module()
class ObjectRefineTransFusionHead(TransFusionHead):
    """Two-stage proposal refinement on top of BEVFusion queries.

    Stage 1: normal TransFusion coarse proposal prediction.
    Stage 2: sample a local fused-BEV patch around each proposal and predict
    residual box deltas from proposal-level features.
    """
    ...
    def __init__(self,
        ...
        self.refine_num_convs = refine_num_convs
        self._refine_common_heads_cfg = common_heads
        self._refine_conv_cfg = conv_cfg
        self._refine_norm_cfg = norm_cfg
        self._refine_bias = bias
        ...
        hidden_channel = self.class_encoding.out_channels

        refine_common_heads = {}
        for k, v in self._refine_common_heads_cfg.items():
            out_c = v[0] if isinstance(v, (tuple, list)) else v
            refine_common_heads[k] = (out_c, self.refine_num_convs)

        self.refine_fuse = nn.Sequential(
            nn.Conv1d(hidden_channel * 2, hidden_channel, kernel_size=1, bias=False),
        ...
        for _ in range(self.num_decoder_layers):
            self.refine_heads.append(
                SeparateHead(
                    hidden_channel,
                    refine_common_heads,
                    conv_cfg=self._refine_conv_cfg,
                    norm_cfg=self._refine_norm_cfg,
                    bias=self._refine_bias,
                )
            )

    def _build_patch_grid(self, centers, H, W):
        """centers: [B, 2, N] in BEV feature coordinates."""
        r = self.patch_radius
        ...
        offsets = torch.stack([xs.reshape(-1), ys.reshape(-1)], dim=-1)  # [K2, 2]
        centers = centers.permute(0, 2, 1)  # [B, N, 2]

        grid = centers[:, :, None, :] + offsets[None, None, :, :]  # [B, N, K2, 2]

        if W > 1:
            grid_x = grid[..., 0] / (W - 1) * 2 - 1
        else:
            grid_x = torch.zeros_like(grid[..., 0])

        if H > 1:
            grid_y = grid[..., 1] / (H - 1) * 2 - 1
        else:
            grid_y = torch.zeros_like(grid[..., 1])

        return torch.stack([grid_x, grid_y], dim=-1)  # [B, N, K2, 2]

    def forward_single(self, inputs, metas):
        batch_size = inputs.shape[0]
        fusion_feat = self.shared_conv(inputs)  # [B, C, H, W]
        ...
        H, W = fusion_feat.shape[2], fusion_feat.shape[3]
        ret_dicts = []
        ...
        for i in range(self.num_decoder_layers):
            ...
            base_res = self.prediction_heads[i](query_feat)

            coarse_center = base_res['center'] + query_pos.permute(0, 2, 1)

            patch_grid = self._build_patch_grid(coarse_center, H, W)
            # grid_sample output: [B, C, N, K2]
            patch_feat = F.grid_sample(
                fusion_feat,
                patch_grid,
                mode='bilinear',
                padding_mode='zeros',
            ...
            patch_feat = patch_feat.mean(dim=-1)  # [B, C, N]

            refine_feat = self.refine_fuse(torch.cat([query_feat, patch_feat], dim=1))
            delta_res = self.refine_heads[i](refine_feat)

            for key in ['height', 'dim', 'rot', 'vel']:
                if key in base_res and key in delta_res:
                    base_res[key] = base_res[key] + self.refine_weight * delta_res[key]

            if 'center' in delta_res:
                base_res['center'] = coarse_center + self.refine_weight * delta_res['center']
            else:
                base_res['center'] = coarse_center

            ret_dicts.append(base_res)
            query_pos = base_res['center'].detach().clone().permute(0, 2, 1)
